...
gsw_summary=function(city,conn){
  ...
  result=DBI::dbReadTable(conn,"gsw_patches_summary") %>%
...
#' @export
gsw_summary_plot=function(gsw_summary_result){
  result_by_reach_zone=gsw_summary_result %>%
    dplyr::group_by(reach,zone) %>%
    dplyr::summarise(propwater=unique(propwater))
  plot=ggplot2::ggplot(gsw_summary_result,
    ...
    ggplot2::geom_point(data=gsw_summary_result %>% dplyr::filter(main==TRUE),
    ...
    ggplot2::xlab("")+ggplot2::ylab("")+ggplot2::labs(title=unique(gsw_summary_result$city)) +
